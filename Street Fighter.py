...
class Fighter():
    def __init__(self, player, x, y, data, sprite_sheet, animations, flip, sound):
...
fighter_1 = Fighter(1, 150,500, knight_data, knight_sheet, knight_animations, False, sword)
fighter_2 = Fighter(2, 1050,500, martial_data, martial_sheet, martial_animations, True, sword)
background = bg()
...
while not finished:
    ...
    if game_over == False:
        ...
        if score[0] == 2 and end_count > 0:
            write_text('HERO KNIGHT WINS', font, RED, SCREEN_WIDTH/8, SCREEN_HEIGHT/2)
        ...
        if score[1] == 2 and end_count > 0:
            write_text('MARTIAL HERO WINS', font, RED, SCREEN_WIDTH/8, SCREEN_HEIGHT/2)
            if pygame.time.get_ticks() - round_over_time >= 1000:
                end_count -= 1
                round_over_time = pygame.time.get_ticks()
            if end_count == 1:
